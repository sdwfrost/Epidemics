#' SIS_Gillespie function
#'
#' Takes parameters for an SIR Infectious Process and returns either an event table or
#' panel data.

SIS_Gillespie = function(N, a, beta, gamma, trans = possible_transitions(1:2), obs_end = Inf,
                         kernel = NULL, U = NULL, E = NULL, output = "event", obs_times = NULL){
  ...
  while(Y[no_event] > 0 & current_time < obs_end){
    ...
    event = event.epidemics(individual_inf_rate, gamma, Y[no_event], U[no_event])
